...
| **Author** | Jesam Kim |
| **License** | MIT |
| **기본 모델 (범용 위임)** | `auto` (CLI 기본값, task-routed, 1.00x credits) |
| **기본 모델 (코드 리뷰)** | 리뷰어 체인 중 첫 사용 가능 모델 — `gpt-5.6-sol` 우선 (§코드 리뷰 특화 참고) |
| **검증 기준** | kiro-cli 2.12.1 (2026-07, 바이너리 직접 introspection + 실측) |
...
**기본 리뷰어 체인(역량 우선 + cross-family 선호).** 이 스킬의 일반 흐름에서는 Claude Code가
작성 모델이므로, 리뷰까지 Claude 계열이 맡으면 같은 모델 패밀리가 자기 작업을 승인하게 됩니다.
그래서 `run_kiro_review.sh`는 리뷰어를 폴백 체인
`gpt-5.6-sol → claude-fable-5 → qwen3-coder-480b → gpt-5.5 → qwen3-coder-next → deepseek-3.2`
중 첫 사용 가능 모델로 해석합니다. 선두의 `gpt-5.6-sol`은 frontier급 리뷰 역량이면서
non-Claude라 두 목표를 동시에 만족합니다. 2순위 `claude-fable-5`는 리뷰 역량만으로 올린
**의도적 same-family 예외**로, 체인이 이를 선택하면 스크립트가 same-family 안내를 출력합니다
(cross-family 속성을 리뷰어 강도와 맞바꾸는 것임을 알고 쓰는 것). 나머지 꼬리는 기존 non-Claude
순서 그대로이며 품질 순위가 아닙니다. `--model <id>`로 임의 모델을 pin 할 수 있고, Claude
계열을 지정하면 스크립트가 same-family self-review 경고를 낸 뒤 그대로 실행합니다. 체인 모델을
찾지 못하면(또는 probe 실패 시) `auto`로 폴백하는데, `auto`가 Claude로 라우팅될 수 있어 이 경우
cross-family가 보장되지는 않습니다.
...
kiro-cli 2.11.0은 `/goal`(성공 기준을 인용 증거로 검증한 뒤 완료하는 goal-driven 자율
...
**범용 위임**의 CLI 실제 기본값은 `auto`(1.00x credits, task별 라우팅)이며 `run_kiro.sh`도
이를 따릅니다. 반면 **코드 리뷰**(`run_kiro_review.sh`)는 리뷰어 체인
`gpt-5.6-sol → claude-fable-5 → qwen3-coder-480b → gpt-5.5 → qwen3-coder-next → deepseek-3.2`
중 첫 사용 가능 모델을 고릅니다(선두 2개는 리뷰 역량 순, 꼬리는 기존 non-Claude 순서 유지).
`--model`로 임의 모델을 pin 할 수 있고(Claude 계열을
지정하면 same-family 경고 후 실행), 체인의 **어떤 모델도** 사용 불가하면(또는 probe 실패 시)
`auto`로 폴백합니다 — 이때 `auto`가 Claude로 라우팅될 수 있어 cross-family가 보장되지는
않습니다. 자세한 근거는
위의 **코드 리뷰 특화** 절을 참고하세요. `--model`은 결정성이나 특정 티어가 필요할 때만 지정하고,
`claude-opus-4.8`은 2.2x credits이므로 일상 작업에 하드코딩하지 마세요.
...
### v2.2.0
- **리뷰어 체인 개편** (`run_kiro_review.sh`, kiro-cli 2.12.1 기준): GPT-5.6 지원에 따라
  체인을 `gpt-5.6-sol → claude-fable-5 → qwen3-coder-480b → gpt-5.5 → qwen3-coder-next
  → deepseek-3.2`로 갱신. 선두 `gpt-5.6-sol`은 frontier 리뷰 역량 + non-Claude를 동시
  만족, 2순위 `claude-fable-5`는 역량 기준의 의도적 same-family 예외(선택 시
  `[chain-same-family]` 태그 + 안내 출력). 꼬리는 기존 non-Claude 순서 유지.
- 검증 기준을 kiro-cli 2.12.1 (2026-07)로 갱신.

### v2.1.0
- **코드 리뷰 기본 리뷰어를 non-Claude cross-family로 전환** (`run_kiro_review.sh`):
  체인 `qwen3-coder-480b → gpt-5.5 → qwen3-coder-next → deepseek-3.2` 중 첫 사용 가능
  모델을 사용(순서는 품질 순위 아님). `--model`로 override(Claude 계열 지정 시 same-family
  경고), non-Claude 미발견 시 `auto` 폴백(cross-family 미보장).
- 검증 기준을 kiro-cli 2.11.0 (2026-07)로 갱신.
