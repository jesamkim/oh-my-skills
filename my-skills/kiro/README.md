...
| **기본 모델 (코드 리뷰)** | non-Claude cross-family 체인 중 첫 사용 가능 모델 (§코드 리뷰 특화 참고) |
| **검증 기준** | kiro-cli 2.11.0 (2026-07, 바이너리 직접 introspection + 실측) |
...
**cross-family 기본 리뷰어(왜 non-Claude 기본인가).** 이 스킬의 일반 흐름에서는 Claude Code가
작성 모델이므로, 리뷰까지 Claude 계열이 맡으면 같은 모델 패밀리가 자기 작업을 승인하게 됩니다.
그래서 `run_kiro_review.sh`는 리뷰어를 **non-Claude 모델로 기본 설정**합니다 — 폴백 체인
`qwen3-coder-480b → gpt-5.5 → qwen3-coder-next → deepseek-3.2` 중 첫 사용 가능 모델을 씁니다
(순서는 품질 순위가 아니며, 각 항목은 단지 non-Claude일 뿐입니다). `--model <id>`로 임의 모델을
pin 할 수 있고, Claude 계열을 지정하면 스크립트가 same-family self-review 경고를 낸 뒤 그대로
실행합니다. non-Claude 모델을 찾지 못하면(또는 probe 실패 시) `auto`로 폴백하는데, `auto`가
Claude로 라우팅될 수 있어 이 경우 cross-family가 보장되지는 않습니다.
...
이를 따릅니다. 반면 **코드 리뷰**(`run_kiro_review.sh`)는 non-Claude cross-family 모델을 기본으로
씁니다 — 체인 `qwen3-coder-480b → gpt-5.5 → qwen3-coder-next → deepseek-3.2` 중 첫 사용 가능
모델을 고릅니다(순서는 품질 순위가 아님). `--model`로 임의 모델을 pin 할 수 있고(Claude 계열을
지정하면 same-family 경고 후 실행), cross-family 모델을 찾지 못하면 `auto`로 폴백합니다 —
이때 `auto`가 Claude로 라우팅될 수 있어 cross-family가 보장되지는 않습니다. 자세한 근거는
