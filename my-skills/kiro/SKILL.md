...
Default model (code review): first available of the reviewer chain (see Mode 2)
```

**Model choice matters (verified on kiro-cli 2.12.1).** For **general delegation**
...
resolves the reviewer through a capability-ranked chain led by `gpt-5.6-sol`
(see Mode 2). Pin a model
...
### Default reviewer chain (capability-ranked head, cross-family preference)
...
family* — `run_kiro_review.sh` resolves its reviewer through a fixed chain that
**leads with the strongest non-Claude frontier model**.

- **Fallback chain** (first available wins): `gpt-5.6-sol → claude-fable-5 →
  qwen3-coder-480b → gpt-5.5 → qwen3-coder-next → deepseek-3.2`.
  - `gpt-5.6-sol` leads: frontier-tier review capability AND cross-family
    (non-Claude), so it satisfies both goals at once.
  - `claude-fable-5` is a **deliberate same-family exception** ranked second
    purely on review capability. When the chain picks it, the script prints a
    same-family note — you are trading the cross-family property for reviewer
    strength, knowingly.
  - The tail keeps the legacy non-Claude order (`qwen3-coder-480b → gpt-5.5 →
    qwen3-coder-next → deepseek-3.2`); there the order carries no quality
    ranking — every entry is simply non-Claude, ordered by
    familiarity/availability, since the region's model list is dynamic.
...
- **Fallback:** if no chain model is available (or the probe fails), the
...
- The startup log tags the outcome: `[cross-family]`, `[chain-same-family]`
  (chain resolved to `claude-fable-5`), `[user-override]`, `[user-auto]`, or
  `[auto-fallback]`.
...
# Claude's own code). Omit --model to get the chain default (gpt-5.6-sol first) instead.
```
...
2.12.1): `--effort`, `--agent`, granular `--trust-tools`, and the `auto` model default.
